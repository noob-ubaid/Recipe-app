...
const DashboardRecipeDetails = () => {
  ...
  const handleLike = () => {
    setLike((prevLike) => {
      ...
      fetch(`https://recipe-book-server-rho-gilt.vercel.app/addrecipes/${recipe._id}`, {
        method: "PUT",
        headers: {
      ...
      <div className="border flex justify-between mt-6 md:mt-12 dark:border-gray-700 items-start gap-8 flex-col md:flex-row border-[#0F0F0F26] p-6 md:p-8 rounded-md">
        ...
        <div className=" w-full md:w-[60%]">
          <h3 className=" text-xl dark:text-white font-semibold">
            <span className="font-semibold">Recipe Name</span> : {recipe.name}
          </h3>
          <p className="text-[#0F0F0F99] dark:text-white font-medium mt-2">
            <span className="font-extrabold">Preparation Time </span> : {recipe.time}{" "}
            Minutes
          </p>
          <p className="text-[#0F0F0F99] dark:text-white font-medium mt-2">
            <span className="font-extrabold">Cuisine Type </span> : {recipe.cuisine}
          </p>
          <p className="text-[#0F0F0F99] dark:text-white font-medium mt-2">
            <span className="font-extrabold"> Category </span> : {recipe.category}
          </p>
          <p className="text-[#0F0F0F99] dark:text-white font-medium mt-2">
          ...
          <p className="text-[#0F0F0F99] dark:text-white font-medium mt-2">
            <span className="font-extrabold"> Instructions </span> :{" "}
            {recipe.instructions}
          </p>
          <div>
            <button
